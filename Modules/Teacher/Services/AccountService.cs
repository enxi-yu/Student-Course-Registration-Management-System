...
namespace StudentCourse.Services
{
    public sealed class AccountService
    {
        ...
        public object ChangePassword(ChangePasswordRequest request)
        {
            ...
            string? storedPassword = _accountRepository.GetPasswordHash(session.UserId);
            bool changed = storedPassword is not null
                && PasswordHash.Verify(storedPassword, request.OldPassword.Trim(), out _)
                && _accountRepository.UpdatePassword(session.UserId, PasswordHash.Hash(request.NewPassword.Trim()));
