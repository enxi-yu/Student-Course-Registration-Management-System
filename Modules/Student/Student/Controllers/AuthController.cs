...
public sealed class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request)
    {
        ...
        if (!request.Username.Trim().StartsWith("S", StringComparison.OrdinalIgnoreCase)) return BadRequest(new { message = "学生账号必须以 S 开头" });

        const string sql = @"SELECT u.user_id, u.username, u.password, u.real_name, u.status, s.student_no, s.major, s.grade FROM ""user"" u JOIN student s ON s.user_id = u.user_id WHERE UPPER(u.username) = UPPER(:username) AND u.role_id = 0";
